...
| Piece | Role |
|---|---|
| `ctx.tower` (`TowerService`) | Mode `enter` / `exit` / `isActive`, rate-limit, roster cache |
| `@deepseek-ai/schemastery` Config | `experimental`, `inflightCap`, `announceToAgent` |
| Mode injection | full (`/tower on`), sparse (after spawn), exit (`/tower off` / teardown) |
| Worker profile deny | orchestration tools + `AskUserQuestion` + `TodoList` |
| Reviewer deny | above + write/edit tools |
| TodoList while mode active | denied via `tools/pre-execute` for main |

See [`PLAN.md`](./PLAN.md) for the gap analysis vs Kimi / DSH idioms.
...
| Seam | Use |
|---|---|
| `ctx.tower` Service | Mode latch, rate limit, roster cache (`declare module` typed) |
| `ctx.tools.register` + `defineTool` | 11 `Tower*` tools |
| `ctx.tools.guard` | Sync write veto for workers |
| `ctx.on('tools/pre-execute')` | Async out-of-worktree deny + TodoList deny in mode |
...
| `ctx.effect` | Clear mode / roster / rate-limit on fiber dispose |
...
src/host/              # apply, TowerService, tools, /tower, spawn, guard
src/host/injection/    # full / sparse / exit reminders (.dsh-tower)
tests/protocol/
tests/host/
PLAN.md                # Cordis-native gap plan
```
