...
export class TowerStore {
  ...
  async addWorktree(worktree: string, branch: string, base: string): Promise<string> {
    // dsh-tower divergence from Kimi (#3346): refuse dirty-base worktree creation.
    if (await isWorktreeDirty(this.repoRoot)) {
      throw new TowerProtocolError(
        'refusing to create a worktree while the base checkout is dirty — commit or stash first',
      );
    }
